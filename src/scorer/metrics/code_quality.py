...
def is_allowed(filename: str) -> bool:
    ...
    for part in parts:
        if part.lower() in EXCLUDED_DIRS: return False
    _, ext = os.path.splitext(filename)
    if ext.lower() in EXCLUDED_EXTS: return False
    return True

def run_radon(path: str) -> float:
    """Run radon for Python maintainability."""
    cmds = [[sys.executable, "-m", "radon", "mi", "-s", path], ["radon", "mi", "-s", path]]
    result = None
    for cmd in cmds:
        try:
...
def score_from_lizard_totals(totals: dict) -> float:
    ...
    if avg_ccn <= 10: ccn_score = 1.0
    elif avg_ccn <= 15: ccn_score = 0.8
    elif avg_ccn <= 25: ccn_score = 0.5
    else: ccn_score = 0.2

    avg_nloc = totals.get("Avg NLOC", 0)
    if avg_nloc <= 40: nloc_score = 1.0
    elif avg_nloc <= 60: nloc_score = 0.8
    elif avg_nloc <= 120: nloc_score = 0.5
    else: nloc_score = 0.2
...
def _check_code_repo_quality(code_url: str) -> float:
    ...
    try:
        ...
        for root, _, files in os.walk(temp_dir):
            if "test" in root.lower(): reliability = 0.7
            for f in files:
                if "test" in f.lower(): reliability = 0.7
                if f.lower() in ["makefile", "cmakelists.txt", "pom.xml", "build.gradle"]: reliability = max(reliability, 0.7)
        if reliability == 0.0: reliability = 0.3 # Base reliability for existing code
        ...
        # 3. Testability
        testability = 0.0
        ci_files = [".github", ".gitlab-ci.yml", "azure-pipelines.yml", ".travis.yml", "circle.yml"]
        for ci in ci_files:
        ...
        build_files = [
            ...
            "package.json", "Makefile", "CMakeLists.txt", "pom.xml", "build.gradle", 
            "Cargo.toml", "go.mod"
        ]
